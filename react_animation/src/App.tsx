...
function App(){
  ...
  return (
    <Wrapper>
      <Grid>
        {[1,2,3,4].map(n=><Box key={n}  onClick={()=>setId(n+"")} layoutId={n+""} />)}
      </Grid>
      <AnimatePresence>
        {id?<Overlay 
        onClick={()=>setId(null)}
        initial={{backgroundColor:"rgba(0,0,0,0)"}} animate={{backgroundColor:"rgba(0,0,0,0.5)"}} exit={{backgroundColor:"rgba(0,0,0,0)"}}>
