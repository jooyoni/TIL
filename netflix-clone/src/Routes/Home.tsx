...
function Home(){
    ...
    return <Wrapper>{isLoading? <Loader></Loader> : (
        <>
            ...
            <Slider>
                <AnimatePresence initial={false} onExitComplete={toggleLeaving}>
                    ...
                    exit="exit" transition={{type:"linear", duration:1}} key={index}>
                        {data?.results.slice(1).slice(offset*index,offset*index+offset)
                        .map((movie)=>(
                            <Box key={movie.id} variants={BoxVariants} layoutId={movie.id+""}
                            onClick={()=> onBoxClicked(movie.id)} 
                            whileHover="hover"  initial="normal" transition={{type:"linear"}}
